...
class LGEJob(DeployerInterface): #pylint:disable=too-many-instance-attributes
    """Handle all the k8s' template building for a training"""

    def __init__(self, namespace=None, runs=1, output=None,
                 cleanup=True, labels=None, job_name=None, stream_log=True,
                 deployer_type=constants.JOB_DEPLOYER_TYPE, manual_selector=False, label_selector_mutators=None,
        ...
        :param manual_selector:  new Job since you are not using the selector
        :param label_selector_mutators: the client/user can identify a set of objects
        ...
        self.label_selector_mutators = label_selector_mutators or []
    ...
    def deploy(self, pod_spec): #pylint:disable=arguments-differ
        ...
        label_selector_spec = self.generate_label_selector_spec()
        for fn in self.label_selector_mutators:
            fn(label_selector_spec)
        ...
        self.deployment_spec = self.generate_deployment_spec(label_selector_spec,pod_template_spec)
        if self.output:
    ...
    def generate_deployment_spec(self, label_selector_spec, pod_template_spec):
        ...
        if not isinstance(label_selector_spec, k8s_client.V1LabelSelector):
            raise TypeError("""label_selector_spec must be a V1LabelSelector,
                            but got %s""" % type(label_selector_spec))

        job_spec = k8s_client.V1JobSpec(
            manual_selector=self.manual_selector,
            selector=label_selector_spec,
